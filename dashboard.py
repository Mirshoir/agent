...
if nav_option == tr("nav_dashboard"):
    ...
    if not business:
        ...
        if not daily_rows and not post_rows:
            ...
            latest_followers = int(df_daily["followers_count"].iloc[-1]) if not df_daily.empty and "followers_count" in df_daily and len(df_daily) else 0
            ...
            if not df_daily.empty:
                df_daily["insight_date"] = pd.to_datetime(df_daily["insight_date"])
                chart_df = df_daily.set_index("insight_date")
                if "reach" in chart_df.columns:
                    st.subheader(tr("daily_reach"))
                    st.bar_chart(chart_df[["reach"]])
                if "followers_count" in chart_df.columns:
                    st.subheader(tr("daily_followers"))
                    st.line_chart(chart_df[["followers_count"]])

            if not df_posts.empty:
                st.subheader(tr("top_posts"))
                columns = ["caption", "media_type", "likes_count", "comments_count", "reach", "impressions", "saved", "shares", "engagement_rate", "permalink"]
