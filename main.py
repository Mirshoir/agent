import osimport timeimport secretsimport requestsfrom urllib.parse import urlencode

from fastapi import FastAPI, Requestfrom fastapi.responses import PlainTextResponse, JSONResponse, RedirectResponsefrom supabase import create_client
...
VERIFY_TOKEN = os.getenv("VERIFY_TOKEN", "1234")MISTRAL_API_KEY = os.getenv("MISTRAL_API_KEY")SUPABASE_URL = os.getenv("SUPABASE_URL")SUPABASE_SERVICE_KEY = os.getenv("SUPABASE_SERVICE_KEY")META_APP_ID = os.getenv("META_APP_ID")META_APP_SECRET = os.getenv("META_APP_SECRET")
...
if not SUPABASE_URL:raise RuntimeError("Missing SUPABASE_URL")

if not SUPABASE_SERVICE_KEY:raise RuntimeError("Missing SUPABASE_SERVICE_KEY")
...
processed_comment_ids: dict = {}processed_message_ids: dict = {}DEDUP_TTL_SECONDS = 60 * 60  # 1 hour
...
def normalize_id(value) -> str:return str(value or "").strip()

def safe_token(token: str) -> str:if not token:return ""token = str(token)if len(token) <= 18:return token[:4] + "..."return token[:10] + "..." + token[-6:]

def cleanup_dedup_cache():now = time.time()for cache in (processed_comment_ids, processed_message_ids):expired = [key for key, created_at in cache.items()if now - created_at > DEDUP_TTL_SECONDS]for key in expired:cache.pop(key, None)

def already_processed(cache: dict, event_id: str) -> bool:if not event_id:return Falsecleanup_dedup_cache()if event_id in cache:return Truecache[event_id] = time.time()return False

def sanitize_business_row(row: dict):clean = dict(row)clean["access_token"] = safe_token(clean.get("access_token", ""))return clean
...
def get_business(instagram_business_id: str):instagram_business_id = normalize_id(instagram_business_id)if not instagram_business_id:return None
...
def find_business_for_webhook(entry_id: str, recipient_id: str = ""):entry_id = normalize_id(entry_id)recipient_id = normalize_id(recipient_id)
...
def exchange_code_for_token(code: str) -> dict:"""Exchange the Instagram OAuth code for a short-lived Instagram User token.Returns dict with access_token and user_id."""res = requests.post("https://api.instagram.com/oauth/access_token",data={"client_id": META_APP_ID,"client_secret": META_APP_SECRET,"grant_type": "authorization_code","redirect_uri": REDIRECT_URI,"code": code,},timeout=30,)print("Instagram OAuth token status:", res.status_code)print("Instagram OAuth token response:", res.text)res.raise_for_status()return res.json()

def exchange_for_long_lived_token(short_lived_token: str) -> str:"""Exchange a short-lived Instagram token (~1 hour) for a long-lived one (~60 days).Uses graph.instagram.com with ig_exchange_token grant.This is the correct endpoint for Instagram Business Login tokens.Falls back to short-lived token on error."""try:res = requests.get(f"https://graph.instagram.com/access_token",params={"grant_type": "ig_exchange_token","client_secret": META_APP_SECRET,"access_token": short_lived_token,},timeout=30,)print("Long-lived token status:", res.status_code)print("Long-lived token response:", res.text)
...
def get_instagram_profile(access_token: str, fallback_user_id: str = "") -> dict:"""Fetch the Instagram Business profile for the authenticated user.Uses graph.instagram.com/me — correct for Instagram Business Login tokens."""try:res = requests.get("https://graph.instagram.com/me",params={"fields": "id,user_id,username,account_type","access_token": access_token,},timeout=30,)print("Profile status:", res.status_code)print("Profile response:", res.text)
...
def subscribe_to_instagram_webhooks(access_token: str, instagram_user_id: str) -> dict:"""Subscribe this Instagram account to receive webhook events (messages, comments).
...
def upsert_connected_business(profile: dict, access_token: str):instagram_business_id = normalize_id(profile.get("user_id"))username = profile.get("username") or f"instagram_{instagram_business_id}"
...
def is_catalog_request(text: str) -> bool:text = (text or "").lower()keywords = ["catalog", "katalog", "каталог","price", "narx", "narxi", "цена", "прайс","cost", "how much", "qancha", "сколько",]return any(keyword in text for keyword in keywords)

def get_catalog_link(business: dict) -> str:catalog_link = business.get("catalog_link")if catalog_link:return catalog_linkknowledge = business.get("knowledge", "")if "https://bitly.cx/eIbT0" in knowledge:return "https://bitly.cx/eIbT0"return ""

def build_catalog_dm(business: dict) -> str:catalog_link = get_catalog_link(business)if not catalog_link:return ("Katalog havolasi hozircha mavjud emas. ""Tezroq ma'lumot olish uchun sales manager bilan bog'lanishingiz mumkin.")return (f"Katalogni shu havola orqali ko'rishingiz mumkin:\n{catalog_link}\n\n""Qo'shimcha ma'lumot kerak bo'lsa, yozib qoldiring 😊")

def build_business_context(business: dict) -> str:return f"""Business name:{business.get("business_name", "")}
...
def get_ai_reply(user_text: str, business: dict) -> str:if not MISTRAL_API_KEY:return ("Xabaringiz qabul qilindi. ""Hozir avtomatik javobda texnik muammo bor, tez orada javob beramiz.")
...
try:res = requests.post("https://api.mistral.ai/v1/chat/completions",headers={"Authorization": f"Bearer {MISTRAL_API_KEY}","Content-Type": "application/json",},json={"model": "mistral-small-latest","messages": [{"role": "system", "content": system_prompt},{"role": "user", "content": user_text},],"temperature": 0.4,"max_tokens": 220,},timeout=30,)print("Mistral result:", res.status_code, res.text)
...
except Exception as e:print("Mistral error:", str(e))return ("Xabaringiz qabul qilindi. ""Hozir avtomatik javobda texnik muammo bor, tez orada javob beramiz.")
...
def send_dm(access_token: str, recipient_id: str, text: str):recipient_id = normalize_id(recipient_id)if not access_token or not recipient_id or not text:print("Cannot send DM: missing token, recipient_id, or text")return None
...
def reply_to_comment(access_token: str, comment_id: str, text: str):comment_id = normalize_id(comment_id)if not access_token or not comment_id or not text:print("Cannot reply comment: missing token, comment_id, or text")return None
...
async def process_messaging_event(entry_id: str, messaging: dict):print("Raw messaging event:", messaging)
...
async def process_comment_event(entry_id: str, change: dict):if change.get("field") != "comments":return
...
async def process_message_change_event(entry_id: str, change: dict):if change.get("field") != "messages":return
...
@app.get("/")async def home():return {"status": "ok","version": "instagram_business_login_with_subscription","note": "Clients only need an Instagram Business/Creator account — no Facebook required","endpoints": {"connect_instagram": "/connect-instagram","debug_business_ids": "/debug/business-ids","debug_businesses": "/debug/businesses","debug_find_business": "/debug/find-business?entry_id=&recipient_id=","debug_check_subscription": "/debug/check-subscription?instagram_business_id=","debug_resubscribe": "/debug/resubscribe?instagram_business_id=",},}

@app.get("/debug/business-ids")async def debug_business_ids():result = (supabase.table("businesses").select("id,business_name,instagram_business_id,bot_enabled,access_token,catalog_link,created_at").order("created_at", desc=True).execute())rows = []for row in result.data or []:rows.append({"id": row.get("id"),"business_name": row.get("business_name"),"instagram_business_id": row.get("instagram_business_id"),"bot_enabled": row.get("bot_enabled"),"has_access_token": bool(row.get("access_token")),"access_token_preview": safe_token(row.get("access_token", "")),"catalog_link": row.get("catalog_link"),"created_at": row.get("created_at"),})return {"count": len(rows), "rows": rows}

@app.get("/debug/businesses")async def debug_businesses():result = (supabase.table("businesses").select("*").order("created_at", desc=True).execute())rows = [sanitize_business_row(row) for row in (result.data or [])]return {"count": len(rows), "businesses": rows}

@app.get("/debug/find-business")async def debug_find_business(entry_id: str = "", recipient_id: str = ""):business = find_business_for_webhook(entry_id, recipient_id)return {"entry_id": entry_id,"recipient_id": recipient_id,"matched": bool(business),"business": sanitize_business_row(business) if business else None,}

@app.get("/debug/check-subscription")async def debug_check_subscription(instagram_business_id: str = ""):"""Check the current webhook subscription status for an Instagram account."""if not instagram_business_id:return JSONResponse({"error": "instagram_business_id is required"}, status_code=400)
...
@app.get("/debug/resubscribe")async def debug_resubscribe(instagram_business_id: str = ""):"""Manually re-trigger the Instagram webhook subscription for an existing account.Useful for fixing accounts connected before the subscription step was added."""if not instagram_business_id:return JSONResponse({"error": "instagram_business_id is required"}, status_code=400)
...
@app.get("/connect-instagram")async def connect_instagram():"""Redirect clients to Instagram Business Login OAuth.
...
@app.get("/auth/callback")async def auth_callback(request: Request):code = request.query_params.get("code")error = request.query_params.get("error")error_description = request.query_params.get("error_description")
...
@app.get("/webhook")async def verify_webhook(request: Request):params = request.query_paramsif (params.get("hub.mode") == "subscribe"and params.get("hub.verify_token") == VERIFY_TOKENand params.get("hub.challenge")):return PlainTextResponse(params.get("hub.challenge"), status_code=200)return PlainTextResponse("Verification failed", status_code=403)

@app.post("/webhook")async def receive_webhook(request: Request):try:data = await request.json()print("Received webhook:", data)
...
@app.get("/privacy")async def privacy():return PlainTextResponse("Privacy Policy: This app collects Instagram messages and comments to provide ""automated AI replies. No personal data is sold or shared with third parties.")

@app.get("/terms")async def terms():return PlainTextResponse("Terms of Service: This app provides automated Instagram replies using AI. ""Responses may not always be accurate. This service is provided as is.")
