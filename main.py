...
GRAPH_VERSION = os.getenv("GRAPH_VERSION", "v21.0")GRAPH_BASE = f"https://graph.facebook.com/{GRAPH_VERSION}"
...
processed_comment_ids = {}processed_message_ids = {}DEDUP_TTL_SECONDS = 60 * 60
...
def cleanup_dedup_cache():now = time.time()for cache in (processed_comment_ids, processed_message_ids):expired = [k for k, v in cache.items() if now - v > DEDUP_TTL_SECONDS]for key in expired:cache.pop(key, None)
...
def sanitize_business_row(row: dict):if not row:return Noneclean = dict(row)clean["access_token"] = safe_token(clean.get("access_token", ""))return clean
...
def get_business_by_page_id(page_id: str):page_id = normalize_id(page_id)if not page_id:return None
...
def exchange_facebook_code_for_token(code: str) -> str:res = requests.get(f"{GRAPH_BASE}/oauth/access_token",params={"client_id": META_APP_ID,"client_secret": META_APP_SECRET,"redirect_uri": FACEBOOK_REDIRECT_URI,"code": code,},timeout=30,)print("Facebook OAuth token status:", res.status_code)print("Facebook OAuth token response:", res.text)res.raise_for_status()return res.json()["access_token"]

def exchange_for_long_lived_facebook_token(short_token: str) -> str:try:res = requests.get(f"{GRAPH_BASE}/oauth/access_token",params={"grant_type": "fb_exchange_token","client_id": META_APP_ID,"client_secret": META_APP_SECRET,"fb_exchange_token": short_token,},timeout=30,)print("Long-lived Facebook token status:", res.status_code)print("Long-lived Facebook token response:", res.text)
...
def get_facebook_pages(user_access_token: str):res = requests.get(f"{GRAPH_BASE}/me/accounts",params={"fields": "id,name,access_token,instagram_business_account{id,username,name}","access_token": user_access_token,},timeout=30,)print("Pages status:", res.status_code)print("Pages response:", res.text)res.raise_for_status()return res.json().get("data", [])

def subscribe_page_to_webhooks(page_id: str, page_access_token: str) -> dict:res = requests.post(f"{GRAPH_BASE}/{page_id}/subscribed_apps",params={"access_token": page_access_token,"subscribed_fields": "messages,messaging_postbacks,feed",},timeout=30,)print("Page subscription status:", res.status_code)print("Page subscription response:", res.text)
...
def upsert_connected_business_from_page(page: dict):page_id = normalize_id(page.get("id"))page_name = page.get("name") or f"page_{page_id}"page_token = page.get("access_token")
...
def get_catalog_link(business: dict) -> str:catalog_link = business.get("catalog_link")if catalog_link:return catalog_link
...
def build_catalog_dm(business: dict) -> str:catalog_link = get_catalog_link(business)
...
try:res = requests.post("https://api.mistral.ai/v1/chat/completions",headers={"Authorization": f"Bearer {MISTRAL_API_KEY}","Content-Type": "application/json",},json={"model": "mistral-small-latest","messages": [{"role": "system", "content": system_prompt},{"role": "user", "content": user_text},],"temperature": 0.4,"max_tokens": 220,},timeout=30,)
...
def send_dm(access_token: str, recipient_id: str, text: str):recipient_id = normalize_id(recipient_id)
...
def reply_to_comment(access_token: str, comment_id: str, text: str):comment_id = normalize_id(comment_id)
...
async def process_comment_event(entry_id: str, change: dict):field = change.get("field")if field not in ["comments", "feed"]:return
...
@app.get("/")async def home():return {"status": "ok","version": "facebook_page_oauth_instagram_bot","connect_facebook": "/connect-facebook","connect_instagram_alias": "/connect-instagram","webhook": "/webhook","debug": {"business_ids": "/debug/business-ids","businesses": "/debug/businesses","find_business": "/debug/find-business?entry_id=&recipient_id=","resubscribe": "/debug/resubscribe?instagram_business_id=",},}

@app.get("/connect-instagram")async def connect_instagram_alias():return RedirectResponse("/connect-facebook")

@app.get("/connect-facebook")async def connect_facebook():if not META_APP_ID:return PlainTextResponse("Missing META_APP_ID", status_code=500)
...
@app.get("/auth/facebook/callback")async def facebook_callback(request: Request):code = request.query_params.get("code")error = request.query_params.get("error")error_description = request.query_params.get("error_description")
...
@app.get("/debug/business-ids")async def debug_business_ids():result = (supabase.table("businesses").select("id,business_name,instagram_business_id,facebook_page_id,facebook_page_name,bot_enabled,access_token,catalog_link,created_at").order("created_at", desc=True).execute())
...
@app.get("/debug/businesses")async def debug_businesses():result = (supabase.table("businesses").select("*").order("created_at", desc=True).execute())

rows = [sanitize_business_row(row) for row in (result.data or [])]
return {"count": len(rows), "businesses": rows}

@app.get("/debug/find-business")async def debug_find_business(entry_id: str = "", recipient_id: str = ""):business = find_business_for_webhook(entry_id, recipient_id)
...
@app.get("/debug/resubscribe")async def debug_resubscribe(instagram_business_id: str = ""):if not instagram_business_id:return JSONResponse({"error": "instagram_business_id is required"}, status_code=400)
...
@app.get("/webhook")async def verify_webhook(request: Request):params = request.query_params
