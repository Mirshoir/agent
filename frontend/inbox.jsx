...
const WORKSPACE_TEXT = {
  en: {
    ...
    instagramDmTestMode: 'Instagram DM test mode', instagramDmTestModeHint: 'Requires Bot enabled and Instagram DMs on. Then AI replies only to usernames or IDs in the allowlist below.',
  ...
  uz: {
    ...
    instagramDmTestMode: 'Instagram DM test rejimi', instagramDmTestModeHint: 'Bot yoqilgan va Instagram DM yoqilgan bo‘lishi kerak. Shunda AI faqat pastdagi username yoki IDlarga javob beradi.',
  ...
  ru: {
    ...
    instagramDmTestMode: 'Тестовый режим Instagram DM', instagramDmTestModeHint: 'Требует включенные Bot enabled и Instagram DM. Тогда ИИ отвечает только username или ID из списка ниже.',
    ...
    <section className="workspace-panel">
      ...
      {view === 'settings' && (
        <div className="settings-view">
          ...
          />
          <label className="field-row">
            <span>{w.instagramDmTestAllowlist}</span>
            <input
              value={instagramDmTestAllowlist || ''}
              placeholder={w.instagramDmTestAllowlistPlaceholder}
              onChange={(e) => onInstagramDmTestAllowlist(e.target.value, false)}
              onBlur={(e) => onInstagramDmTestAllowlist(e.target.value, true)}
            />
            <small>{w.instagramDmTestAllowlistHint}</small>
          </label>
          <label className="field-row">
...
function App({ lang, setLang, onSignOut, onAuthExpired, currentUser }) {
  ...
  const loadWorkspaceState = async (businessId = selectedBusinessId) => {
    ...
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(state, 'instagram_dm_test_allowlist')) {
        const allowlist = typeof state.instagram_dm_test_allowlist === 'string'
          ? state.instagram_dm_test_allowlist
          : Array.isArray(state.instagram_dm_test_allowlist)
            ? state.instagram_dm_test_allowlist.join(', ')
            : '';
        setInstagramDmTestAllowlist(allowlist);
      } else {
  ...
  const updateInstagramDmTestAllowlist = (value, persist = false) => {
    const next = String(value || '');
    setInstagramDmTestAllowlist(next);
    if (persist) {
