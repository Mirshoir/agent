...
function normalizeConversation(row) {
  const name = row.customer_name || `Client ${String(row.customer_id || '').slice(-4)}`;
  const unread = Number(row.unread_count || 0);
  const total = Number(row.total_messages || 0);
  const platform = row.platform || 'instagram';
  const channel = row.channel || '';
  const channelName = channelLabel(platform, channel);
  return {
    id: row.id,
    apiId: row.id,
    businessId: row.business_id,
    customerId: row.customer_id,
    chatId: row.chat_id || row.customer_id,
    channel,
    channelName,
    name,
    handle: platformHandle(row),
    platform,
    avatar: avatarFor(name, row.customer_id),
    online: false,
    needsHuman: unread > 0,
    aiOn: unread === 0,
    unread,
    lastTime: formatRelative(row.last_message_at),
    lastFromMe: false,
    preview: row.last_message || 'No message preview',
    tags: [platform, channelName].filter(Boolean),
    customerSince: 'first message',
    location: channelName,
    summary: `${total || 1} saved message${total === 1 ? '' : 's'} in this ${channelName} conversation. Replies send through ${sendRouteFor({ platform, channel })}.`,
    kpis: { orders: total, ltv: String(unread), last: formatRelative(row.last_message_at) || '—', conv: unread ? `${unread} unread` : 'read' },
    orders: [],
    suggestions: [],
  };
}
...
function normalizeMessage(row, index) {
  const inbound = row.direction === 'inbound' || row.role === 'user';
  const media = row.media_type;
  const text = row.content || '';
  let type = 'text';
  if (media === 'voice' || media === 'audio') type = 'voice';
  ...
  const message = {
    ...
    text,
    mediaKind: media || '',
    mediaUrl: withMediaToken(row.media_url || telegramUserMediaUrl(row)),
...
function WorkspacePanel({ view, stats, businesses, selectedBusinessId, onSelectBusiness, onRefresh, onBusinessSetting, onToast }) {
        ...
        <div className="settings-view">
          ...
          <div className="settings-section">
            ...
            <label className="field-row prompt-row">
              <span>Active model prompt</span>
              <textarea
                value={selectedBusiness.ai_reply_rules || ''}
                onChange={(e) => onBusinessSetting(selectedBusiness.id, { ai_reply_rules: e.target.value }, false)}
                onBlur={(e) => onBusinessSetting(selectedBusiness.id, { ai_reply_rules: e.target.value }, true)}
                rows={7}
              />
            </label>
          </div>
...
function TopBar({ t, lang, setLang, theme, setTheme, conv, aiOn, activeView, onToggleAi, onRefresh, onToast, onPin, onArchive, onMore, moreOpen }) {
  ...
  const workspaceNames = { inbox: 'Inbox', insights: 'Insights', knowledge: 'Knowledge', accounts: 'Accounts', settings: 'Settings', profile: 'Profile' };
  return (
...
function App() {
  ...
  const refreshWorkspace = async () => {
    await Promise.all([loadConversations({ sideLoad: false }), loadStats(), loadBusinesses()]);
  ...
  const loadThread = async (conversationId, { silent = false } = {}) => {
    ...
    try {
      const data = await API.get(`/api/conversation/${encodeURIComponent(conversationId)}`);
      setThreads(prev => ({
        ...prev,
      ...
      API.post(`/api/conversation/${encodeURIComponent(conversationId)}/read`).catch(() => {});
      setApiError('');
      return true;
    } catch (e) {
  ...
  const sendLiveMessage = async (conversation, text) => {
    if (conversation.platform === 'telegram' && conversation.channel === 'telegram_user_private') {
      const result = await API.postJson('/dashboard/send-telegram-user-message', {
    ...
    const result = await API.post('/api/send-message', { conversation_id: conversation.apiId || conversation.id, text });
  ...
  const toggleAi = async () => {
    ...
    try {
      await API.post('/api/chat-ai-toggle', {
  ...
  const changeView = (view) => {
    setActiveView(view);
    const names = { inbox: 'Inbox', insights: 'Insights', knowledge: 'Knowledge', accounts: 'Accounts', settings: 'Settings', profile: 'Profile' };
  ...
  useEffect(() => {
    ...
    if (liveMode) {
      API.post(`/api/conversation/${encodeURIComponent(selectedId)}/read`).catch(() => {});
    }
  }, [selectedId, liveMode]);
